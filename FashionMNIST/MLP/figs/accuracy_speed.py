...
for i in range(len(accuracies0)):
    accuracies3.append(0.8983)

names = range(len(accuracies0)-20)
#x = range(len(names))
x0 = range(len(accuracies0)-20)
x1 = range(len(accuracies1)-20)
x2 = range(len(accuracies2)-20)
x3 = range(len(accuracies3)-20)

#plt.plot(x, y, 'ro-')
#plt.plot(x, y1, 'bo-')
plt.xlim(0, time_steps-20)  
plt.ylim(0, 1.0) 
plt.plot(x0, accuracies0[:20], marker='o', ms=8,label='k=0: accuracy=88.9%, time steps=5')
plt.plot(x1, accuracies1[:20], marker='*', ms=8,label='k=1: accuracy=88.79%, time steps=11')
plt.plot(x2, accuracies2[:20], marker='^', ms=8,label='k=2: accuracy=90.11%, time steps=20')
plt.plot(x3, accuracies3[:20], marker='_', ms=8,label='Full-precision ANN: accuracy=89.83%') # need to be updated
plt.legend(loc='lower right', prop=fonten)  
#text(60, -0.01, u'Di', style='italic', fontdict=fonten) 
plt.xticks(x0, names, rotation=45)
plt.margins(0)
plt.subplots_adjust(bottom=0.15)
plt.xlabel("Time step", fontproperties=roman) 
plt.ylabel("Accuracy", fontproperties=roman) 
plt.title("Accuracy versus Speed on FashionMNIST", fontproperties=roman) 

plt.show()

plt.savefig('smlp_accuracy_fashionmnist.svg',format='svg')
