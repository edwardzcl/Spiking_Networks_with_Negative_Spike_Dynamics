...
def load_n_mnist_dataset(shape=(-1, 1156), path='data'):
    """Load the converted N-MNIST from "mat" format.

    You should firstly download original files at https://www.garrickorchard.com/datasets/n-mnist.
    Then, run the Matlab script to make a simple spike-to-image conversion.
    Finally, this Python function automatically load N_MNIST dataset from Mat format and return the training, validation and test set with 50000, 10000 and 10000 digit images respectively.

    Parameters
    ----------
    shape : tuple
        The shape of digit images (the default is (-1, 1156), alternatively (-1, 34, 34, 1)).
    path : str
        The path that the data is downloaded to.
    ...
    Examples
    --------
    >>> X_train, y_train, X_test, y_test = tl.files.load_mnist_dataset(shape=(-1,1156), path='datasets')
    >>> X_train, y_train, X_test, y_test = tl.files.load_mnist_dataset(shape=(-1, 34, 34, 1))
    """
    n_mnist = scio.loadmat('data/n_mnist/N_MNIST.mat')
    X_train = n_mnist['n_mnist']['train_data'][0,0].transpose(2,0,1)[:,:,:,np.newaxis]
    y_train = n_mnist['n_mnist']['train_label'][0,0]
    X_test = n_mnist['n_mnist']['test_data'][0,0].transpose(2,0,1)[:,:,:,np.newaxis]
    y_test = n_mnist['n_mnist']['test_label'][0,0]
    
    X_train = np.asarray(X_train, dtype=np.float32) / np.float32(32)
    y_train = np.asarray(y_train, dtype=np.int32).reshape(-1)
    X_test = np.asarray(X_test, dtype=np.float32) / np.float32(32)
