...
plt.ylim(0, 1.0)  
plt.plot(x0, accuracies0, marker='o', ms=8,label='k=0: accuracy=92.73%, time steps=26')
plt.plot(x1, accuracies1, marker='*', ms=8,label='k=1: accuracy=93.35%, time steps=40')
plt.plot(x2, accuracies2, marker='^', ms=8,label='k=2: accuracy=93.10%, time steps=89')
plt.plot(x3, accuracies3, marker='_', ms=8,label='Full-precision ANN: accuracy=93.20%')  # need to be updated
plt.legend(loc='lower right', prop=fonten) 
#text(60, -0.01, u'Di', style='italic', fontdict=fonten) 
plt.xticks(x0[::5], names[::5], rotation=45)
plt.margins(0)
plt.subplots_adjust(bottom=0.15)
plt.xlabel("Time step", fontproperties=roman) 
plt.ylabel("Accuracy", fontproperties=roman) 
plt.title("Accuracy versus Speed on CIFAR10", fontproperties=roman) 

plt.show()

plt.savefig('scnn2_accuracy_cifar10.svg',format='svg')
