...
class MyNet():
    def __init__(self):
        ...
        self.conv1 = Conv2d(in_channels=1, n_filter=12, filter_size=(5, 5), padding=0, stride=1, use_ternary=False)   
        self.bn1 = BatchNorm2d(n_channel=12, momentum=0.1)
        self.relu1 = Relu()
        
        self.conv2 = Conv2d(in_channels=12, n_filter=12, filter_size=(2, 2), padding=0, stride=2, use_ternary=False)
        self.bn2 = BatchNorm2d(n_channel=12, momentum=0.1)
        self.relu2 = Relu()

        self.conv3 = Conv2d(in_channels=12, n_filter=64, filter_size=(4, 4), padding=0, stride=1, use_ternary=False)
        self.bn3 = BatchNorm2d(n_channel=64, momentum=0.1)
        self.relu3 = Relu()

        self.conv4 = Conv2d(in_channels=64, n_filter=64, filter_size=(2, 2), padding=0, stride=2, use_ternary=False)
        self.bn4 = BatchNorm2d(n_channel=64, momentum=0.1)
        self.relu4 = Relu()

        self.flatten = Flatten()
        
        # ȫ���Ӳ�
        self.fc1 = Linear(dim_in=2304, dim_out=10, use_ternary=False)

        
        self.parameters = self.conv1.params + self.bn1.params + self.conv2.params + self.bn2.params + \
                            self.conv3.params + self.bn3.params + self.conv4.params + self.bn4.params + self.fc1.params
        
        self.dummy_layers = [self.conv1, self.bn1, self.conv2, self.bn2, self.conv3, self.bn3, self.conv4, self.bn4, self.fc1]                                        
    ...
    def forward(self, X, t, mode):
        ...
        flat_out = self.flatten(conv4_spike_collect, t)

        # the last layer output the membrane potential value indexing category  
        fc1_out = self.fc1(flat_out, t)

        # spike number
        spike_num = conv1_spike_num + conv2_spike_num + conv3_spike_num + conv4_spike_num
        # spike collector  
        spike_collect = np.sum(conv1_spike_collect) + np.sum(conv2_spike_collect) + np.sum(conv3_spike_collect) + np.sum(conv4_spike_collect)
        # synaptic operations  
        sop_num = conv2_sop_num + conv3_sop_num + conv4_sop_num + conv4_spike_num * 10
...
# load parameter
model = np.load('model_n_mnist.npz')
params = model['params']
