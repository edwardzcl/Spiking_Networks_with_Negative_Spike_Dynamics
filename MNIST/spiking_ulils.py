...
class Conv2d():
    count = 0
    def __init__(self, in_channels, n_filter, filter_size, padding, stride, use_ternary):
        """
        ...
        self.params = [self.W, self.b, self.bound, self.diff]
        self.type = 'conv'
    ...
    def __call__(self, X, time_step):
        ...
        if time_step == 0:
            ...
            self.mem_q = np.repeat(np.repeat(np.repeat(self.bound[:, np.newaxis], self.w_out, axis=1)[:, np.newaxis, :], \
    ...
    def forward(self, X, time_step):
        ...
        # compute difference
        diff = np.maximum(self.mem, np.repeat(np.repeat(np.repeat(self.bound[:, np.newaxis], self.w_out, axis=1)[:, np.newaxis, :], \
            self.h_out, axis=1)[np.newaxis, :, :, :], self.n_x, axis=0)) - self.mem_q
        # generate spikes    
        self.spike_out = np.where(diff > np.repeat(np.repeat(np.repeat(self.diff[:, np.newaxis], self.w_out, axis=1)[:, np.newaxis, :], \
            self.h_out, axis=1)[np.newaxis, :, :, :], self.n_x, axis=0), 1, np.where(diff < -1e-5, -1, 0))
        # update shadow membrane potential
        self.mem_q = np.where(diff > np.repeat(np.repeat(np.repeat(self.diff[:, np.newaxis], self.w_out, axis=1)[:, np.newaxis, :], \
            self.h_out, axis=1)[np.newaxis, :, :, :], self.n_x, axis=0), self.mem_q + np.repeat(np.repeat(np.repeat(self.diff[:, np.newaxis], \
            self.w_out, axis=1)[:, np.newaxis, :], self.h_out, axis=1)[np.newaxis, :, :, :], self.n_x, axis=0), np.where(diff < -1e-5, \
            self.mem_q - np.repeat(np.repeat(np.repeat(self.diff[:, np.newaxis], self.w_out, axis=1)[:, np.newaxis, :], \
            self.h_out, axis=1)[np.newaxis, :, :, :], self.n_x, axis=0), self.mem_q))   
